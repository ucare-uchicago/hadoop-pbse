...
FROM = table.JOBS
WHERE = lambda job: (job["isInvolveSlownode"] and float(job["job_duration"]) > 180) #or job["tags_PBSE"]!=[]
...
FROM = table.TASKS
WHERE = lambda task: task["appid"].endswith("_0113")
SELECT = lambda task: [task["attempt"][-10:], task["containerid"][-5:], task["lastDatanode"][:5], task["mapnode"][:5], \
                       ...
                       task["time_start"], task["status_update"][0], task["time_stop"]]
