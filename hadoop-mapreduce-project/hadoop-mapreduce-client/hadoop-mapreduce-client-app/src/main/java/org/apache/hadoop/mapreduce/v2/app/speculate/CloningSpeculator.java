...
public class CloningSpeculator extends AbstractService implements
    Speculator {
  ...
  private static final Log LOG = LogFactory.getLog(CloningSpeculator.class);
  ...
  public CloningSpeculator(Configuration conf, AppContext context) {
    this(conf, context, context.getClock());
  }

  public CloningSpeculator(Configuration conf, AppContext context, Clock clock) {
  ...
  public CloningSpeculator
      (Configuration conf, AppContext context,
       TaskRuntimeEstimator estimator, Clock clock) {
    super(CloningSpeculator.class.getName());
  ...
  private synchronized void processSpeculatorEvent(SpeculatorEvent event) {
    switch (event.getType()) {
      ...
      case ATTEMPT_START:
      {
        ...
        // riza: make the clone immediately
        if (event.getReportedStatus().id.getId() <= 0) {
          LOG.info("riza: we are cloning task " + event.getTaskID());
          addSpeculativeAttempt(event.getTaskID());
        }
        break;
      }
