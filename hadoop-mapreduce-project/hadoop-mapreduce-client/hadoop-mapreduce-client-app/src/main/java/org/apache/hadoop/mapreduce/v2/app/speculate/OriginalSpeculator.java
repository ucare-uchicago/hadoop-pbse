...
public class OriginalSpeculator extends AbstractService implements
    Speculator {
  ...
  private static final Log LOG = LogFactory.getLog(OriginalSpeculator.class);
  ...
  public OriginalSpeculator(Configuration conf, AppContext context) {
    this(conf, context, context.getClock());
  }

  public OriginalSpeculator(Configuration conf, AppContext context, Clock clock) {
  ...
  public OriginalSpeculator
      (Configuration conf, AppContext context,
       TaskRuntimeEstimator estimator, Clock clock) {
    super(OriginalSpeculator.class.getName());
...
/*   *************************************************************    */
  ...
  protected void serviceStart() throws Exception {
    ...
    speculationBackgroundThread = new Thread
        (speculationBackgroundCore, "OriginalSpeculator background processing");
...
//  those jobs that need them.
  ...
  protected void addSpeculativeAttempt(TaskId taskID) {
    LOG.info
        ("OriginalSpeculator.addSpeculativeAttempt -- we are speculating " + taskID);
